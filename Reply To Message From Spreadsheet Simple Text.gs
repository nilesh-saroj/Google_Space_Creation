...
var replyText = {
  ...
  if (messageResponse.getResponseCode() === 200) {
    ...
    sheet.getRange(lastProcessedRow + index + 1, 13).setValue('Sent');
  } else {
    sheet.getRange(lastProcessedRow + index + 1, 13).setValue("Message not sent : " + messageResponse.getContentText());
